...
//
// 対応イベント:
//   - checkout.session.completed                : card / paypay など即時決済。payment_status が paid の時だけ処理する
//   - checkout.session.async_payment_succeeded   : konbini など非同期決済が実際に完了した時
//   - checkout.session.async_payment_failed      : konbini の期限切れなど、支払いが失敗した時(ログのみ)
...
// --- 注文保存・明細保存・在庫減算をまとめた処理 ---
// checkout.session.completed(即時決済)と async_payment_succeeded(konbini等)の
// 両方から呼び出すため、共通関数として切り出す。
async function fulfillOrder(sql, session) {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      // card / paypay は即時決済なので payment_status が paid ならここで確定処理。
      // konbini はここでは unpaid のままなので、後続の async_payment_succeeded を待つ。
      if (session.payment_status === 'paid') {
        await fulfillOrder(sql, session);
      } else {
        console.log(
          `[SESSION] payment_status=${session.payment_status} のため確定処理を保留(非同期決済の完了待ち)。session_id=${session.id}`
        );
      }
    } else if (event.type === 'checkout.session.async_payment_succeeded') {
      // konbiniなど、店頭での支払いが実際に完了した時にここが発火する
      const session = event.data.object;
      console.log('[ASYNC PAYMENT SUCCEEDED] session_id:', session.id);
      await fulfillOrder(sql, session);
    } else if (event.type === 'checkout.session.async_payment_failed') {
      // konbiniの支払い期限切れなど。注文は作らず、ログだけ残す。
      const session = event.data.object;
      console.warn('[ASYNC PAYMENT FAILED] session_id:', session.id, '支払いが完了しませんでした(期限切れ等)');
    } else {
